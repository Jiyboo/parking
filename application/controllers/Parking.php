...
class Parking extends Admin_Controller 
{
    ...
    'plate_number' => $this->input->post('plate_number'), // Tambahkan ini
...
public function printInvoice($id)
{
    ...
    if ($id) {
        ...
        $plate_number = isset($parking_data['plate_number']) ? $parking_data['plate_number'] : 'N/A';


        // Nama file unik untuk QR Code
        $qr_filename = FCPATH . 'assets/qrcode/Qrcode ' . $parking_code . '.png';

        $params['data'] = $parking_code;
        $params['level'] = 'H';
        $params['size'] = 5;
        $params['savename'] = $qr_filename;
        $this->ciqrcode->generate($params);

        // Tunggu sampai file QR Code benar-benar tersedia
        $max_attempts = 5;
        $attempt = 0;
        while (!file_exists($qr_filename) && $attempt < $max_attempts) {
            sleep(1);
            $attempt++;
        }

        if (file_exists($qr_filename)) {
            $qr_base64 = base64_encode(file_get_contents($qr_filename));
            $qr_src = 'data:image/png;base64,' . $qr_base64;
        } else {
            $qr_src = '';
        }

        // HTML Cetak Kartu Parkir
        $html = '<html>
        <head>
            ...
            <script>
                function printAfterLoad() {
                    console.log("QR Code siap, mencetak...");
                    setTimeout(() => {
                        window.print();
            ...
            <div class="container">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td><strong>Plate Number</strong> ' . $plate_number . '</td>
                    </tr>
                </table>
                <br><br>
                <img class="qrcode" src="' . $qr_src . '" alt="QR Code" onload="printAfterLoad()">
